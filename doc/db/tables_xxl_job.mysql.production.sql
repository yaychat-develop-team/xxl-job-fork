-- XXL-JOB 3.4.0 production schema for the existing xxl_job database.
-- Apply once through Bytebase. This intentionally contains no demo jobs,
-- demo executors, or password-based administrator account.
...
(
    ...
    `job_group`                 int(11) NOT NULL COMMENT '执行器主键ID',
    `job_id`                    int(11) NOT NULL COMMENT '任务，主键ID',
    `executor_address`          varchar(255) DEFAULT NULL COMMENT '本次执行地址',
    `executor_handler`          varchar(255) DEFAULT NULL COMMENT '任务handler',
    `executor_param`            text DEFAULT NULL COMMENT '任务参数',
...
(
    ...
    `suc_count`     int(11) NOT NULL DEFAULT '0' COMMENT '成功日志数量',
    `fail_count`    int(11) NOT NULL DEFAULT '0' COMMENT '失败日志数量',
    `update_time`   datetime DEFAULT NULL,
    PRIMARY KEY (`id`),
...
(
    `id`         int(11)      NOT NULL AUTO_INCREMENT,
    `username`   varchar(254) NOT NULL COMMENT 'Cloudflare Access email',
    `password`   varchar(100) NOT NULL COMMENT '不可登录的随机密码哈希',
    `token`      varchar(100) DEFAULT NULL COMMENT '登录token',
    `role`       tinyint(4)   NOT NULL COMMENT '角色：0=普通用户、1=管理员',
    `permission` varchar(255) DEFAULT NULL COMMENT '执行器ID列表',
    PRIMARY KEY (`id`),
